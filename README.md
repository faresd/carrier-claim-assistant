...
The browser cache, delivery audits, and claim outcomes use the same seller-account/marketplace/order identity as the cloud database. When Amazon omits the merchant ID from a page URL, the extension derives a stable provisional identity from the visible Seller Central account-switcher name. The server upgrades that provisional identity when Amazon later exposes the real merchant ID, without duplicating the order or losing its tracking and claim history. It also repairs pre-existing generic/merchant duplicates when the ownership is unambiguous, while preserving claim references, carrier events, notification receipts, and active launch links; ambiguous cross-account matches remain separate. Version 2.9 also automatically migrates earlier order-number-only cache entries, preventing identical order numbers from different Amazon accounts from overwriting one another.
...
While La Poste Suivi v2 approval is pending, the server first attempts the documented v1 API. If both API calls fail, paired installations cooperatively process the failed morning queue through one reusable inactive official tracking-page tab. Server leases prevent different Chrome/Brave profiles from checking the same parcel at the same time, and a fresh browser result is required before a lease can be completed.
...
Administrators can export the private order and tracking history as a JSON backup. Resolved records can be permanently deleted with explicit confirmation; active pickup, return, and investigation cases are protected from deletion. A privacy-minimal server marker prevents old paired browsers from restoring a deleted record.

When a dashboard-started claim succeeds, the paired browser briefly opens the saved Amazon order in one inactive tab, appends the claim reference to Seller Notes, and closes the tab. The write is bounded and best-effort; if Amazon is signed out or the notes control is temporarily unavailable, the saved claim outcome is applied automatically the next time that order page is opened.
