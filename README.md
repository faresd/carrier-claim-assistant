...
The browser cache, delivery audits, and claim outcomes use the same seller-account/marketplace/order identity as the cloud database. Version 2.9 automatically migrates earlier order-number-only cache entries, preventing identical order numbers from different Amazon accounts from overwriting one another.
